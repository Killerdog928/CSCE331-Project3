...
/**
 * HomePage component for the cashier application.
 *
 * This component allows users to select meal options, customize their meals with sides, entrees, appetizers, and drinks,
 * and add the selected items to a cart. It also includes feature filters to include or exclude specific item features.
 *
 * State Variables:
 * - itemFeatures: List of item features fetched from the API.
 * - includeFeatures: List of features to include in the item search.
 * - excludedFeatures: List of features to exclude from the item search.
 * - showAllMealOptions: Boolean to toggle the visibility of all meal options.
 * - showFeatureFilters: Boolean to toggle the visibility of feature filters.
 * - sides, entrees, appetizers, drinks: Lists of items fetched from the API.
 * - loading: Boolean to indicate if data is being loaded.
 * - selectedSides, selectedEntrees, selectedAppetizers, selectedDrinks: Records to keep track of selected item quantities.
 * - selectedEntreeType: The currently selected meal type.
 * - cartItems: List of items added to the cart.
 * - currentCartIndex: Index of the current cart item.
 *
 * Effects:
 * - Fetches item features from the API on component mount.
 * - Fetches sides, entrees, appetizers, and drinks based on selected features and meal type.
 *
 * Functions:
 * - getQueryParams: Constructs query parameters for API requests based on included and excluded features.
 * - getMaxQuantity: Returns the maximum quantity allowed for a given feature in the selected meal type.
 * - handleSideIncrement, handleSideDecrement: Handlers to increment/decrement the quantity of selected sides.
 * - handleEntreeIncrement, handleEntreeDecrement: Handlers to increment/decrement the quantity of selected entrees.
 * - handleAppetizerIncrement, handleAppetizerDecrement: Handlers to increment/decrement the quantity of selected appetizers.
 * - handleDrinkIncrement, handleDrinkDecrement: Handlers to increment/decrement the quantity of selected drinks.
 * - handleEntreeTypeSelect: Handler to select a meal type and reset selected items.
 * - handleAddToCart: Assembles the selected items into an order structure and adds it to the cart.
 * - handleSubmitOrder: Submits the current order to the API.
 *
 * Render:
 * - Displays feature filters, meal type selection, and item selection sections.
 * - Includes buttons to add items to the cart and submit the order.
 * - Displays the cart section and a weather display component.
 */
const HomePage = () => {
  // State variables
  ...
  const getQueryParams = (
    ...
    if (exclude) {
      params += `${params ? "&" : ""}excludeFeatures=${encodeURIComponent(
        exclude,
      )}`;
    }
